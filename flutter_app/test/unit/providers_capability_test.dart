...
void main() {
  test('mock engines report mock mode and refuse to place calls', () async {
    ...
    for (final e in engines) {
      ...
      await expectLater(() => e.placeCall('+12145550100'), throwsA(isA<UnsupportedError>()));
      await e.dispose();
    }
  });
  ...
  test('demo messaging provider simulates delivery + reply', () async {
    ...
    await p.sendSms(const OutboundMessageRequest(to: '+12145550100', from: '+12145550999', body: 'appointment?'));
  ...
  test('mock messaging maps opt-out keyword', () {
    ...
    final ev = p.mapInboundWebhook({'MessageSid': 'X', 'From': '+1', 'Body': 'STOP'});
    expect(ev.kind, 'opt-out');
  });

  test('AI simulator books, escalates, and honors opt-out — never a real call', () async {
