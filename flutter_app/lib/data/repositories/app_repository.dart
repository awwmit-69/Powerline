...
class AppRepository {
  ...
  void upsertContact(Contact c, {String op = 'update'}) {
    ...
    _update(
      _state.copyWith(contacts: list),
      journalEntity: 'contact',
      journalId: c.id,
      op: op,
      log: '$op contact ${c.displayName}',
    );
  }
  ...
  void deleteContact(String id) {
    _update(
      _state.copyWith(
        contacts: _state.contacts.where((c) => c.id != id).toList(),
      ),
      journalEntity: 'contact',
      journalId: id,
      op: 'delete',
  ...
  void addSmsSuppression(String e164) {
    ...
    _update(
      _state.copyWith(smsSuppression: [..._state.smsSuppression, e164]),
      journalEntity: 'suppression',
      journalId: e164,
      op: 'create',
      log: 'SMS opt-out $e164',
    );
  }

  // ---- Messaging ----
  Conversation ensureConversation(String remoteE164, {String? contactId}) {
    final existing = _state.conversations.where(
      (c) => c.remoteE164 == remoteE164,
    );
    ...
    _update(
      _state.copyWith(conversations: [conv, ..._state.conversations]),
      journalEntity: 'conversation',
      journalId: conv.id,
      op: 'create',
    );
    return conv;
  ...
  void addMessage(Message m) {
    final convs = _state.conversations.map((c) {
      ...
      return c.copyWith(
        ...
        unreadCount: m.direction == CallDirection.inbound
            ? c.unreadCount + 1
            : c.unreadCount,
    ...
    _update(
      _state.copyWith(messages: [..._state.messages, m], conversations: convs),
      journalEntity: 'message',
      journalId: m.id,
      op: 'create',
    );
  }
  ...
  void addCall(CallRecord r) {
    _update(
      _state.copyWith(calls: [r, ..._state.calls]),
      journalEntity: 'call',
      journalId: r.id,
      op: 'create',
      log: 'call ${r.direction.name} ${r.remoteE164}',
    );
  }
  ...
  void updateCall(CallRecord r) {
    _update(
      _state.copyWith(
        calls: _state.calls.map((x) => x.id == r.id ? r : x).toList(),
      ),
      journalEntity: 'call',
      journalId: r.id,
      op: 'update',
  ...
  void addVoicemail(Voicemail v) {
    _update(
      _state.copyWith(voicemails: [v, ..._state.voicemails]),
      journalEntity: 'voicemail',
      journalId: v.id,
      op: 'create',
    );
  ...
  void upsertAppointment(Appointment a) {
    ...
    _update(
      _state.copyWith(appointments: list),
      journalEntity: 'appointment',
      journalId: a.id,
      log: 'appointment ${a.id} ${a.status.name}',
    );
  }
  ...
  void upsertCallback(CallbackTask t) {
    ...
    _update(
      _state.copyWith(callbacks: list),
      journalEntity: 'callback',
      journalId: t.id,
    );
  }
  ...
  void addHandoff(HandoffEvent e) {
    ...
    _update(
      _state.copyWith(handoffs: [..._state.handoffs, e], calls: calls),
      journalEntity: 'handoff',
      journalId: e.id,
      op: 'create',
