...
class Contact {
  ...
  factory Contact.fromJson(Map<String, dynamic> j) => Contact(
        ...
        phones: (j['phones'] as List? ?? [])
            .map(
                (e) => PhoneEntry.fromJson(Map<String, dynamic>.from(e as Map)))
            .toList(),
        emails: _ls(j['emails']),
        tags: _ls(j['tags']),
        ...
        customFields: (j['customFields'] as Map? ?? {}).map(
...
class CallRecord {
  ...
  factory CallRecord.fromJson(Map<String, dynamic> j) => CallRecord(
        ...
        notes: _s(j['notes']),
        finalState:
            _enum(CallState.values, j['finalState'], CallState.completed),
...
class Voicemail {
  ...
  factory Voicemail.fromJson(Map<String, dynamic> j) => Voicemail(
        ...
        transcriptConfidence:
            (j['transcriptConfidence'] as num?)?.toDouble() ?? 0,
        read: _b(j['read']),
        archived: _b(j['archived']),
        notes: _s(j['notes']),
        createdAt: DateTime.tryParse(_s(j['createdAt'])) ?? DateTime.now(),
