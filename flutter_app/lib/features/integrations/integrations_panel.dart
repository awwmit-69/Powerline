...
class IntegrationsPanel extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final s = stateOf(ref);
    final categories = ['telephony', 'messaging', 'ai', 'transcription', 'calendar', 'crm'];

    return ListView(padding: const EdgeInsets.all(20), children: [
      const Text('Provider integrations',
      ...
      for (final cat in categories) ...[
        ...
        for (final card in s.integrations.where((i) => i.category == cat))
          _IntegrationCardView(
              card: card, color: _stateColor(card.state), label: _stateLabel(card.state)),
      ],
    ]);
  }
}
...
class _IntegrationCardView extends ConsumerWidget {
  ...
  final Color color;
  final String label;
  const _IntegrationCardView({required this.card, required this.color, required this.label});
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Text(card.provider, style: const TextStyle(fontWeight: FontWeight.w700)),
            const SizedBox(width: 8),
            Chip(label: Text(label, style: const TextStyle(fontSize: 10))),
            const Spacer(),
            if (card.state == ProviderState.notConfigured || card.state == ProviderState.error)
              TextButton(
                onPressed: () => _configure(context, ref),
                child: const Text('Configure'),
              ),
            if (card.configuredFieldNames.isNotEmpty || card.state == ProviderState.activeDemo)
              TextButton(
          ...
          if (card.capabilities.isNotEmpty)
            Padding(
              ...
              child: Text('Capabilities: ${card.capabilities.join(', ')}',
                  style: const TextStyle(fontSize: 11)),
            ),
          if (card.missingCapabilities.isNotEmpty)
            Text('Missing: ${card.missingCapabilities.join(', ')}',
                style: const TextStyle(fontSize: 11, color: PowerlineColors.stateRinging)),
          if (card.configFields.isNotEmpty)
            Text('Config fields: ${card.configFields.join(', ')}',
                style: const TextStyle(fontSize: 11, color: PowerlineColors.textSecondary)),
          if (card.configuredFieldNames.isNotEmpty)
            Text('Configured (secret) fields: ${card.configuredFieldNames.join(', ')} — values hidden',
                style: const TextStyle(fontSize: 11, color: PowerlineColors.textSecondary)),
          if (card.lastError != null)
            Text('Last error: ${card.lastError}',
                style: const TextStyle(fontSize: 11, color: PowerlineColors.stateFailed)),
          if (card.lastTestAt != null)
            Text('Last test: ${card.lastTestResult ?? 'n/a'} at ${card.lastTestAt}',
                style: const TextStyle(fontSize: 11, color: PowerlineColors.textSecondary)),
          const Text('Docs: (placeholder link — see PROVIDER_ADAPTERS.md)',
  ...
  void _configure(BuildContext context, WidgetRef ref) {
    final repo = ref.read(appRepositoryProvider);
    final controllers = {for (final f in card.configFields) f: TextEditingController()};
    showDialog<void>(
      context: context,
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () {
              ...
              repo.updateIntegration(card.copyWith(
                state: provided.isEmpty ? ProviderState.notConfigured : ProviderState.configured,
