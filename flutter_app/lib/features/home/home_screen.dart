...
class HomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final center = _Column(
      children: [const _QuickDialpad(), _ActiveCallPanel(), _Timeline()],
    );
    final right = _Column(
      children: [
        _ProviderHealth(),
          ...
          child: Wrap(
            ...
            children: [
              ...
              const _StatusChip(
                ...
                label: 'Provider: Demo (simulation)',
                color: PowerlineColors.stateRinging,
              ),
              _AvailabilityChip(
                repo: repo,
                value: s.settings['availability'] ?? 'Available',
              ),
              const Spacer(),
              _TopButton(
                icon: Icons.search,
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (c) => const [
                  PopupMenuItem(
                    value: 'profile',
                    child: Text('AZD Global (demo user)'),
                  ),
        ...
        Wrap(
          ...
          children: [
            ...
            _Metric(
              ...
              '$newVms',
              Icons.voicemail,
              PowerlineColors.stateVoicemail,
              '/voicemail',
            ),
            _Metric(
              'Appointments booked',
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          selectedItemBuilder: (c) =>
              const ['Available', 'Away', 'Do not disturb']
                  .map(
                    (v) => Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
        ...
        children: [
          ...
          if (session != null)
            ...
            _Row(
              ...
              onTap: () => ref
                  .read(callSessionProvider.notifier)
                  .placeDemoCall(lastCall.remoteE164),
            )
          else
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(
                    child: Text(e.$3, style: const TextStyle(fontSize: 12)),
                  ),
                ],
              ),
            ),
    ...
    return _Panel(
      child: Column(
        ...
        children: [
          ...
          _kv(
            'Active (demo)',
            '${byState[ProviderState.activeDemo] ?? 0}',
            PowerlineColors.stateConnected,
          ),
          _kv(
            'Configured',
            '${byState[ProviderState.configured] ?? 0}',
            PowerlineColors.stateHold,
          ),
          _kv(
            'Not configured',
            '${byState[ProviderState.notConfigured] ?? 0}',
            PowerlineColors.stateRinging,
          ),
          _kv(
            'Errors',
            '${byState[ProviderState.error] ?? 0}',
            PowerlineColors.stateFailed,
          ),
          const SizedBox(height: 6),
          const Text(
            'No live telecom connected — all traffic simulated.',
            style: TextStyle(
              fontSize: 11,
              color: PowerlineColors.textSecondary,
            ),
        ...
        children: [
          ...
          for (final a in appts.take(4))
            _Row(
              ...
              subtitle:
                  '${a.startsAt.month}/${a.startsAt.day} ${a.startsAt.hour}:00'
              ...
              builder: (context) {
                ...
                final set = s.calls
                    .where(
                      (c) =>
                          c.campaignId == cp.id &&
                          c.disposition == 'appointment-set',
                    )
                    .length;
                return _Row(
                  title: cp.name,
                  subtitle: '$cpCalls calls · $set appts · ${cp.status.name}',
                );
              },
            ),
