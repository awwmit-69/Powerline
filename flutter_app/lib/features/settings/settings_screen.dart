    ...
    return ListView(padding: const EdgeInsets.all(20), children: [
        ...
        child: Padding(
          ...
          child: Column(children: [
            TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Company profile name'),
            ...
            DropdownButtonFormField<String>(
              ...
              items: const [DropdownMenuItem(value: 'US', child: Text('United States'))],
              onChanged: (v) => repo.setSetting('defaultCountry', v ?? 'US'),
            ),
            const SizedBox(height: 10),
            DropdownButtonFormField<String>(
              initialValue: s.settings['defaultTimeZone'] ?? 'America/Chicago',
              decoration: const InputDecoration(labelText: 'Default time zone'),
              items: const [
                DropdownMenuItem(value: 'America/New_York', child: Text('Eastern')),
                DropdownMenuItem(value: 'America/Chicago', child: Text('Central')),
                DropdownMenuItem(value: 'America/Denver', child: Text('Mountain')),
                DropdownMenuItem(value: 'America/Los_Angeles', child: Text('Pacific')),
              ],
              onChanged: (v) => repo.setSetting('defaultTimeZone', v ?? 'America/Chicago'),
            ),
            const SizedBox(height: 10),
            DropdownButtonFormField<String>(
              initialValue: s.settings['defaultCallerId'] ?? s.numbers.firstOrNull?.id,
              decoration: const InputDecoration(labelText: 'Default caller ID'),
              items: [
                for (final n in s.numbers)
                  DropdownMenuItem(value: n.id, child: Text('${n.label} (${n.e164})')),
              ],
      ...
      Card(
        child: SwitchListTile(
          ...
          value: s.devices.where((d) => d.isThisDevice).firstOrNull?.ringEnabled ?? true,
          onChanged: (v) {
...
class _NumbersPanel extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListView(padding: const EdgeInsets.all(20), children: [
      Row(children: [
        const Text('Phone numbers', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
        const Spacer(),
        FilledButton.tonal(
        ...
        Card(
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Text('${n.label}  ·  ${PhoneNumberUtil.format(n.e164)}',
              ...
              Wrap(spacing: 6, runSpacing: 6, children: [
                ...
                _cap('Fax', false),
                Chip(label: Text('E911: ${n.e911Status}', style: const TextStyle(fontSize: 10))),
                Chip(label: Text('CNAM: ${n.cnamStatus}', style: const TextStyle(fontSize: 10))),
                Chip(label: Text('A2P/10DLC: ${n.a2pStatus}', style: const TextStyle(fontSize: 10))),
                Chip(label: Text('Toll-free: ${n.tollFreeStatus}', style: const TextStyle(fontSize: 10))),
                Chip(label: Text('Provider: ${n.provider}', style: const TextStyle(fontSize: 10))),
              ]),
              const SizedBox(height: 6),
              const Text(
                  'Port / release / E911 / caller-ID / SMS-registration actions are unavailable until a live provider is connected.',
                  style: TextStyle(fontSize: 10, color: PowerlineColors.textSecondary)),
            ]),
          ),
        ),
    ]);
  ...
  Widget _cap(String label, bool on) => Chip(
        avatar: Icon(on ? Icons.check : Icons.close,
            size: 12, color: on ? PowerlineColors.stateConnected : PowerlineColors.stateFailed),
...
class _RoutingPanel extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final s = stateOf(ref);
    return ListView(padding: const EdgeInsets.all(20), children: [
      const Text('Routing & business hours',
      ...
      Card(
        child: Padding(
          padding: const EdgeInsets.all(14),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Routing rules (priority order)',
                style: TextStyle(fontWeight: FontWeight.w700)),
            for (final r in s.routingRules)
              ListTile(
                dense: true,
                leading: Text('${r.priority}',
            ...
            OutlinedButton.icon(
              ...
              onPressed: () async {
                await ref.read(ringSimProvider.notifier).simulateRing('+15005550142');
      ...
      Card(
        child: Padding(
          padding: const EdgeInsets.all(14),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Business hours (${s.businessHours.timeZone})',
                style: const TextStyle(fontWeight: FontWeight.w700)),
            const SizedBox(height: 6),
            ...
            Text('Currently ${s.businessHours.isOpenAt(DateTime.now()) ? 'OPEN' : 'CLOSED'} (device-local approximation)',
...
class _CompliancePanel extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final repo = ref.read(appRepositoryProvider);
    return ListView(padding: const EdgeInsets.all(20), children: [
      const Text('Compliance & E911', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
      const SizedBox(height: 10),
      Card(
        ...
        child: const Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              SizedBox(width: 8),
              Text('E911 — READ CAREFULLY', style: TextStyle(fontWeight: FontWeight.w800)),
            ]),
            SizedBox(height: 6),
            Text('• Powerline demo mode CANNOT place emergency calls. Do not rely on it for 911.\n'
      ...
      Card(
        child: Padding(
          padding: const EdgeInsets.all(14),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('SMS compliance (A2P 10DLC / toll-free)',
                style: TextStyle(fontWeight: FontWeight.w700)),
            const SizedBox(height: 6),
            _kv('Brand registration', 'not-submitted (requires live provider)'),
            _kv('10DLC campaign', 'not-submitted (requires live provider)'),
            _kv('Toll-free verification', 'not-submitted (requires live provider)'),
            _kv('Campaign use case', 'customer care + appointment reminders (draft)'),
            _kv('Opt-in method', 'verbal + web form (draft, unverified)'),
            _kv('Opt-out keywords', 'STOP, UNSUBSCRIBE, CANCEL, END, QUIT (enforced locally)'),
            _kv('Help keywords', 'HELP, INFO'),
            _kv('Sample message',
                'Hi {firstName}, reminder about your inspection {date}. Reply STOP to opt out.'),
            _kv('Privacy policy / Terms', 'placeholder links — set before live use'),
            const SizedBox(height: 6),
            const Text(
                'Nothing has been submitted to any registry. These are local drafts only.',
                style: TextStyle(fontSize: 11, color: PowerlineColors.stateRinging)),
          ]),
        ),
      ),
      const SizedBox(height: 10),
      Card(
        child: Padding(
          padding: const EdgeInsets.all(14),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Suppression & DNC (${s.dnc.length} DNC, ${s.smsSuppression.length} SMS opt-outs)',
                style: const TextStyle(fontWeight: FontWeight.w700)),
            for (final d in s.dnc.take(10))
              ListTile(
                dense: true,
                leading: const Icon(Icons.block, size: 14, color: PowerlineColors.stateFailed),
                title: Text(PhoneNumberUtil.format(d.e164)),
                subtitle: Text('${d.reason} · source: ${d.source}',
                    style: const TextStyle(fontSize: 11)),
              ),
            Row(children: [
              Expanded(
                child: TextField(
                  decoration: const InputDecoration(
                      labelText: 'Add number to DNC (E.164)', isDense: true),
                  onSubmitted: (v) {
      ...
      const Card(
        child: Padding(
          padding: EdgeInsets.all(14),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Legal notices', style: TextStyle(fontWeight: FontWeight.w700)),
            SizedBox(height: 6),
            Text('• You are responsible for lawful calling and messaging (TCPA, TSR, state law).\n'
...
class _DataPanel extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final repo = ref.read(appRepositoryProvider);
    return ListView(padding: const EdgeInsets.all(20), children: [
      const Text('Backup & data', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
      const SizedBox(height: 10),
      Card(
        child: Column(children: [
          ListTile(
            ...
            subtitle: const Text('Creates a timestamped JSON snapshot next to the live data file.'),
            trailing: FilledButton.tonal(
              onPressed: () async {
                final path = await repo.backup();
                if (context.mounted) {
                  ScaffoldMessenger.of(context)
                      .showSnackBar(SnackBar(content: Text('Backup written: $path')));
                }
              },
          ...
          ListTile(
            ...
            subtitle: const Text('Restores the original fictional seed. Your edits are lost.'),
            trailing: OutlinedButton(
              onPressed: () => showDialog<void>(
                context: context,
                builder: (d) => AlertDialog(
                  title: const Text('Reset demo data?'),
                  content: const Text('All local changes will be replaced by fresh demo data.'),
                  actions: [
          ...
          ListTile(
            leading: const Icon(Icons.delete_forever, color: PowerlineColors.stateFailed),
            ...
            trailing: OutlinedButton(
              onPressed: () => showDialog<void>(
                ...
                builder: (d) => AlertDialog(
                  title: const Text('Delete ALL local data?'),
                  content: const Text('This wipes the local store. This cannot be undone.'),
                  actions: [
                    TextButton(onPressed: () => Navigator.pop(d), child: const Text('Cancel')),
                    FilledButton(
                      style:
                          FilledButton.styleFrom(backgroundColor: PowerlineColors.stateFailed),
                      onPressed: () async {
