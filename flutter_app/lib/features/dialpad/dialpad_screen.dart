...
class _DialpadScreenState extends ConsumerState<DialpadScreen> {
  ...
  void _backspace() =>
      setState(() => digits = digits.isEmpty ? '' : digits.substring(0, digits.length - 1));
  ...
  Widget build(BuildContext context) {
    ...
    final matches = digits.length >= 3
        ? s.contacts
            .where((c) =>
                c.phones.any((p) => p.e164.contains(digits.replaceAll(RegExp(r'\D'), ''))) ||
                c.displayName.toLowerCase().contains(digits.toLowerCase()))
            .take(4)
            .toList()
        : <Contact>[];
    final recents = s.calls.take(5).map((c) => c.remoteE164).toSet().take(4).toList();
    callerIdNumberId ??= s.settings['defaultCallerId'] ?? s.numbers.firstOrNull?.id;
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      const Text('Dialpad', style: TextStyle(fontWeight: FontWeight.w700)),
                      Row(children: [
                        const DemoBadge(),
                        const SizedBox(width: 8),
                        Tooltip(
                          message: 'Demo provider: local simulation only',
                          child: Icon(Icons.circle, size: 10, color: PowerlineColors.stateRinging),
                        ),
                      ]),
                    ],
                  ),
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: Text(
                            digits.isEmpty ? 'Enter number' : PhoneNumberUtil.formatPartial(digits),
                            style: TextStyle(
                              fontSize: 24,
                        ...
                        IconButton(
                            tooltip: 'Paste', onPressed: _paste, icon: const Icon(Icons.paste, size: 18)),
                        IconButton(
                            tooltip: 'Backspace',
                    ...
                    Column(children: [
                      for (final m in matches)
                        ListTile(
                          ...
                          subtitle: Text(m.primaryPhone == null
                              ? ''
                              : PhoneNumberUtil.format(m.primaryPhone!)),
                          onTap: () => setState(() => digits = m.primaryPhone ?? digits),
                        ),
                    ]),
                    ...
                    children: [
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          decoration: const InputDecoration(
                              labelText: 'Caller ID (outbound number)', isDense: true),
                          items: [
                            for (final n in s.numbers)
                              DropdownMenuItem(
                                  value: n.id,
                                  child: Text('${n.label} ${PhoneNumberUtil.format(n.e164)}',
                      ...
                      SegmentedButton<String>(
                        segments: const [
                          ...
                          ButtonSegment(value: 'external', label: Text('OS dialer')),
                        ],
                        selected: {mode},
                  ...
                  Row(
                    ...
                    children: [
                      FilledButton.icon(
                        ...
                        onPressed: normalized == null ? null : () => _call(normalized),
                      ...
                      IconButton(
                        tooltip: 'Add contact',
                        onPressed: normalized == null ? null : () => _addContact(normalized),
                    ...
                    Wrap(
                      ...
                      children: [
                        ...
                        for (final r in recents)
                          ActionChip(
                            label: Text(PhoneNumberUtil.format(r),
                                style: const TextStyle(fontSize: 11)),
                            onPressed: () =>
                                setState(() => digits = r.replaceFirst('+1', '')),
                          ),
                      ],
                    ),
  ...
  Future<void> _call(String e164) async {
      ...
      try {
        ...
        if (!ok && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text('No OS telephone handler available — number copied instead.')));
    ...
    await ref.read(callSessionProvider.notifier).placeDemoCall(
          e164,
          fromNumberId: callerIdNumberId,
        );
  }
  ...
  void _addContact(String e164) {
    ...
    ScaffoldMessenger.of(context)
        .showSnackBar(const SnackBar(content: Text('Contact created — edit in Contacts tab')));
  }
}
