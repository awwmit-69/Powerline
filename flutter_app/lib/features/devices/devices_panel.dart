    ...
    return ListView(padding: const EdgeInsets.all(20), children: [
      Row(children: [
        ...
        FilledButton.tonalIcon(
          ...
          onPressed: () =>
              ref.read(ringSimProvider.notifier).simulateRing('+13145550142'),
        ),
      ]),
        ...
        Card(
          ...
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Ring simulation events',
                  style: TextStyle(fontWeight: FontWeight.w700)),
              for (final e in ring)
                Row(children: [
                  Icon(
                    e.status == 'ringing' ? Icons.ring_volume : Icons.check_circle,
                    size: 14,
                  ...
                  Text('${e.deviceName}: ${e.status}', style: const TextStyle(fontSize: 12)),
                ]),
              TextButton(
                  onPressed: () => ref.read(ringSimProvider.notifier).clearEvents(),
          ...
          child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Row(children: [
                      ...
                      if (d.isThisDevice)
                        const Padding(
                          ...
                          child: Chip(
                              label: Text('This device', style: TextStyle(fontSize: 9)),
                      ...
                      if (d.revoked)
                        const Padding(
                          padding: EdgeInsets.only(left: 6),
                          child: Chip(
                              label: Text('Revoked', style: TextStyle(fontSize: 9)),
              ...
              Wrap(spacing: 12, children: [
                _toggle('Ring', d.ringEnabled,
                    (v) => repo.updateDevice(d.copyWith(ringEnabled: v))),
                _toggle('Msg notif', d.messageNotifications,
                    (v) => repo.updateDevice(d.copyWith(messageNotifications: v))),
                _toggle('VM notif', d.voicemailNotifications,
                    (v) => repo.updateDevice(d.copyWith(voicemailNotifications: v))),
                if (!d.isThisDevice)
                  TextButton.icon(
                    icon: const Icon(Icons.logout, size: 14),
                    label: Text(d.revoked ? 'Revoked' : 'Revoke / log out'),
                    onPressed: d.revoked
                        ? null
                        : () => repo.updateDevice(d.copyWith(revoked: true)),
                  ),
              ]),
            ]),
          ),
