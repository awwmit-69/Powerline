...
class _CampaignsScreenState extends ConsumerState<CampaignsScreen> {
  ...
  static const _tabLabels = ['Campaigns', 'Calling queue', 'Pipeline', 'Appointments'];
  ...
  Widget _campaignList(AppState s) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      const Row(children: [
        Text('Campaigns', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
        SizedBox(width: 8),
        ...
        Card(
          child: ListTile(
            ...
            title: Row(children: [
              ...
              Chip(
                label: Text(cp.status.name, style: const TextStyle(fontSize: 10)),
  ...
  Widget _queue(AppState s) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      Text('Queue for "${cp.name}" — ${eligible.length} callable, ${excluded.length} excluded',
        ...
        Card(
          child: ListTile(
            ...
            leading: const Icon(Icons.check_circle_outline,
                color: PowerlineColors.stateConnected, size: 18),
      ...
      if (excluded.isNotEmpty) ...[
        const SizedBox(height: 10),
        const Text('Excluded (with reasons)', style: TextStyle(fontWeight: FontWeight.w700)),
        for (final e in excluded)
          ListTile(
            dense: true,
            leading:
                const Icon(Icons.block, color: PowerlineColors.stateFailed, size: 16),
            title: Text(e.contact.displayName),
            subtitle: Text(e.exclusionReason ?? '', style: const TextStyle(fontSize: 11)),
          ),
      ],
      const Divider(height: 24),
      const Text('Other queues', style: TextStyle(fontWeight: FontWeight.w700)),
      _MiniQueue(
        ...
        for (final st in stages)
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('${st.name} (${s.deals.where((d) => d.stage == st).length})',
              ...
              for (final d in s.deals.where((d) => d.stage == st))
                Card(
                  child: Padding(
                    padding: const EdgeInsets.all(10),
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(_name(s, d.contactId),
                          style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
                      Text('\$${d.value.toStringAsFixed(0)} · ${d.nextAction}',
                          style: const TextStyle(fontSize: 11)),
                      Row(children: [
                        IconButton(
                          tooltip: 'Move left',
                          ...
                          onPressed: st.index == 0
                              ? null
                              : () => repo.upsertDeal(
                                  d.copyWith(stage: stages[st.index - 1])),
                        ),
                        IconButton(
                          tooltip: 'Move right',
  ...
  Widget _appointments(AppState s) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      const Text('Appointments (agenda)', style: TextStyle(fontWeight: FontWeight.w700)),
            ...
            trailing: PopupMenuButton<String>(
              onSelected: (v) {
                switch (v) {
                  ...
                  case 'complete':
                    repo.upsertAppointment(a.copyWith(status: AppointmentStatus.completed));
                  case 'missed':
                    repo.upsertAppointment(a.copyWith(status: AppointmentStatus.missed));
                  ...
                  case 'reschedule':
                    repo.upsertAppointment(a.copyWith(
                        startsAt: a.startsAt.add(const Duration(days: 1)),
                        status: AppointmentStatus.rescheduled));
                  case 'cancel':
                    repo.upsertAppointment(a.copyWith(status: AppointmentStatus.cancelled));
                  case 'remind':
                    repo.notify('appointment', 'Reminder queued (demo)',
              ...
              itemBuilder: (c) => const [
                ...
                PopupMenuItem(value: 'remind', child: Text('Send reminder (demo)')),
                PopupMenuItem(value: 'complete', child: Text('Mark completed')),
                PopupMenuItem(value: 'missed', child: Text('Mark missed → callback')),
                PopupMenuItem(value: 'reschedule', child: Text('Reschedule +1 day')),
      ...
      for (final cb in cbs)
        Card(
          child: ListTile(
            dense: true,
            leading: Icon(cb.done ? Icons.check_circle : Icons.update, size: 18),
            title: Text(_name(s, cb.contactId)),
            subtitle: Text('due ${cb.dueAt} · ${cb.reason}',
                style: const TextStyle(fontSize: 11)),
            trailing: cb.done
                ? null
                : FilledButton.tonal(
                    onPressed: () => repo.upsertCallback(cb.copyWith(done: true)),
...
class _MiniQueue extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ExpansionTile(
        title: Text('$title (${items.length})', style: const TextStyle(fontSize: 13)),
        children: [
          if (items.isEmpty)
            const Padding(padding: EdgeInsets.all(8), child: Text('Empty')),
          for (final i in items)
            ListTile(dense: true, title: Text(i, style: const TextStyle(fontSize: 12))),
        ],
      ),
    );
  }
