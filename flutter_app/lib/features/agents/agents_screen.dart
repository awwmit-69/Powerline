...
class _AgentsScreenState extends ConsumerState<AgentsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final list = ListView(padding: const EdgeInsets.all(12), children: [
      const Row(children: [
        Text('AI Agents', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
        SizedBox(width: 8),
    ...
    final detail = agent == null
        ...
        : ListView(padding: const EdgeInsets.all(16), children: [
            Text(agent.name, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
            ...
            Card(
              child: Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  _kv('System prompt', agent.systemPrompt.isEmpty ? '(default)' : agent.systemPrompt),
                  _kv('Allowed actions', agent.allowedActions.join(', ')),
                  _kv('Forbidden actions', agent.forbiddenActions.join(', ')),
                  _kv('Escalation conditions', agent.escalationConditions.join(', ')),
                  _kv('Handoff destination', agent.handoffDestination),
                  _kv('Recording', agent.recordingEnabled ? 'enabled' : 'disabled'),
                  _kv('Transcription', agent.transcriptionEnabled ? 'enabled' : 'disabled'),
                ]),
              ),
            ),
            ...
            Card(
              child: Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Row(children: [
                    Text('Call simulator', style: TextStyle(fontWeight: FontWeight.w700)),
                    SizedBox(width: 8),
                  ...
                  Row(children: [
                    DropdownButton<SimPersona>(
                      ...
                      items: const [
                        DropdownMenuItem(
                            value: SimPersona.interested, child: Text('Interested caller')),
                        DropdownMenuItem(
                            value: SimPersona.priceShopper, child: Text('Price shopper')),
                        DropdownMenuItem(
                            value: SimPersona.wantsHuman, child: Text('Wants a human')),
                        DropdownMenuItem(
                            value: SimPersona.notInterested, child: Text('Not interested / opt-out')),
                      ],
                      onChanged: (v) => setState(() => persona = v ?? persona),
                    ),
                  ...
                  if (result != null) ...[
                        ...
                        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          Expanded(
                            child: Column(
                                ...
                                children: [
                                  Text(t.text, style: const TextStyle(fontSize: 12)),
                                  if (t.detectedIntent != null)
                                    Text('intent: ${t.detectedIntent}',
                    ...
                    Wrap(spacing: 8, children: [
                      ...
                      Chip(label: Text('Appointment: ${result!.appointmentBooked ? 'booked' : 'no'}')),
                      Chip(label: Text('Escalated: ${result!.escalatedToHuman ? 'yes' : 'no'}')),
                      Chip(label: Text('DNC: ${result!.dncRequested ? 'requested' : 'no'}')),
                    ]),
                  ],
                ]),
              ),
            ...
            Card(
              child: Padding(
                padding: const EdgeInsets.all(12),
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Text('Recent handoff events', style: TextStyle(fontWeight: FontWeight.w700)),
                  for (final h in s.handoffs.take(6))
                    ListTile(
                      dense: true,
                      leading: const Icon(Icons.swap_horiz, size: 16),
                      title: Text('${h.kind.name}: ${h.fromParty} → ${h.toParty}',
    ...
    if (!wide) return ListView(children: [SizedBox(height: 400, child: list), detail]);
    return Row(children: [
      SizedBox(
          width: 380,
          child: DecoratedBox(
  ...
  Future<void> _run(AiAgent agent) async {
    ...
    repo.addCall(CallRecord(
      id: callId,
      direction: agent.role == 'outbound' ? CallDirection.outbound : CallDirection.inbound,
    ...
    if (r.dncRequested) {
      repo.addDnc('+15005550199', reason: 'requested during AI simulation', source: 'ai-agent');
    }
    setState(() {
      result = r;
      running = false;
