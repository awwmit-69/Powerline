...
class AgentSimulator {
  ...
  Future<SimResult> run(AiAgent agent, SimPersona persona) async {
    final transcript = <SimTurn>[
      SimTurn('system', 'SIMULATED CALL — no real number contacted. Agent "${agent.name}" in test mode.'),
      SimTurn('agent', agent.greeting.isEmpty
          ? 'Hello! This is a demo AI assistant. This call is simulated.'
          : agent.greeting),
    ];
    final callerLines = switch (persona) {
      SimPersona.interested => ['Hi, yes I heard about the inspection.', 'Tuesday works, yes.'],
      SimPersona.priceShopper => ['How much does this cost?', 'OK, Thursday could work, yes.'],
      SimPersona.wantsHuman => ['I would rather talk to a human please.'],
      SimPersona.notInterested => ['Not interested, please remove me from your list.'],
    };
    ...
    for (final line in callerLines) {
      ...
      turns.add(('user', line));
      final reply = await llm.complete(systemPrompt: agent.systemPrompt, turns: turns);
      ...
      transcript.add(SimTurn('agent', reply.replaceFirst(RegExp(r'^\[\w+\] '), '')));
  ...
  String _intent(String line) {
    ...
    if (l.contains('cost') || l.contains('much') || l.contains('price')) return 'pricing-question';
